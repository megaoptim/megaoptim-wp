...
<div class="megaoptim-postbox">
    <form class="content-wrapper" method="POST" id="megaoptim-report-export">
        <div class="megaoptim-middle-content">
            <?php if ( $profile_valid ): ?><?php if ( ! $stats->empty_gallery ): ?>
                ...
                <div id="megaoptim-stats">
                    <div class="megaoptim-row">
                        <div class="megaoptim-col megaoptim-col-12 text-center">
                            <h1><?php _e( 'No images found', 'megaoptim' ); ?></h1>
                        </div>
                    </div>
                </div>
            <?php endif; ?>
                            ...
                            <div class="megaoptim-info">
                                <?php if ( $tokens > 0 ): ?>
                                    ...
                                    <?php if ( $stats->empty_gallery ): ?>
                                        <p class="text-center"><?php _e( 'It seems that your media library is empty, once you upload some attachments come back on this page if you don\'t have Auto Optimize option enabled so you can optimize them all from there.', 'megaoptim' ); ?></p>
                                    <?php elseif ( $stats->total_optimized_mixed > 0 && $stats->total_remaining === 0 ): ?>
                                        <p class="text-center"><strong><?php _e( 'Congratulations', 'megaoptim' ); ?>
                                                !</strong> <?php _e( 'Your media library is fully optimized, please come back later when new attachments are available for optimization.', 'megaoptim' ); ?>
                                        </p>
                                    <?php else: ?>
                                        <p>
                                            <?php echo sprintf( __( 'On this screen you can optimize all of your media library attachments. Make sure you check the %s once more if you haven\'t still to make sure it\'s all set up for your needs before start.', 'megaoptim' ), '<a target="_blank" href="' . $settings_url . '">' . __( 'Settings page', 'megaoptim' ) . '</a>' ); ?>
                                        </p>
                        ...
                        <div class="megaoptim-col megaoptim-col-12 text-center">
                            <?php if($tokens>0): ?>
                                <?php if ( $stats->total_remaining > 0 ): ?>
                                    <button id="megaoptim-toggle-optimizer" data-action="megaoptim_optimize_attachment" data-context="media-library" data-next-state="start" data-stop-text="Stop Bulk Optimizer" data-start-text="Start Bulk Optimizer" class="button button-primary button-extra-large">

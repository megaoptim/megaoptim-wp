...
function megaoptim_current_screen() {
	...
	if ( $screen->id === 'upload' ) {
		...
		if ( $mode === 'grid' ) {
			...
			$message        = sprintf( __( 'MegaOptim provides optimization buttons in the list mode that will help you optimize or restore single attachments. %s to switch to %s mode.' ), $list_mode_url, $list_mode_text );
